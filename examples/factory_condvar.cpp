//
// factory_condvar.cpp -- a factory in C++17, with condition variables.
//
// The baseline for factory_guarded.cpp: the same program, written the usual
// way, with one std::thread per worker.
//
...
// while print_lock is held, so there is no deadlock.
//
